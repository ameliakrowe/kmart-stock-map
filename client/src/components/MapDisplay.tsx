...
function computeMapBounds(centerLocation: Coords, searchRadius: number): google.maps.LatLngBoundsLiteral {
    const { lat, lon } = centerLocation;
    const north = computeDestinationPoint({latitude: lat, longitude: lon}, searchRadius * 1000, 0).latitude;
    const south = computeDestinationPoint({latitude: lat, longitude: lon}, searchRadius * 1000, 180).latitude;
    const east = computeDestinationPoint({latitude: lat, longitude: lon}, searchRadius * 1000, 90).longitude;
    const west = computeDestinationPoint({latitude: lat, longitude: lon}, searchRadius * 1000, 270).longitude;
