...
async function binarySearchFromCoords(coord: Coord): Promise<BinarySearchResult> {
    let min = 0;
    let max = 1000;
    let counter = 0;
    ...
    while (max - min > 10) {
        ...
        try {
            const nearestLocationsResponse = await getNearestLocations(coord.lat.toString(), coord.lon.toString(), guess.toString());
            counter += 1;
...
function excludeCoordsToSearch(coords: SearchCoord[], centerCoord: Coord, radius: number): SearchCoord[] {
    coords.forEach((coord: SearchCoord) => {
        const kmFromCenter = (getDistance({latitude: centerCoord.lat, longitude: centerCoord.lon}, {latitude: coord.coord.lat, longitude: coord.coord.lon}))/1000;
...
export async function getAllLocations() {
    ...
    const currentSearchCoord = {lat: coordLimits.north, lon: coordLimits.west};
